...
const games: { type: GameType; title: string; description: string; icon: React.ReactNode; gradient: string; shadowColor: string }[] = [
  ...
  },
  {
    type: 'matching',
    title: 'لعبة التوصيل',
    description: 'وصّل الأسئلة بالإجابات الصحيحة',
  ...
  },
  {
    type: 'true-false',
    title: 'لعبة صح وغلط',
    description: 'حدد إذا كانت المعادلة صحيحة أم خاطئة',
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col min-h-screen p-3 sm:p-4 md:p-6">
        ...
        {/* ========== الشريط العلوي ========== */}
        <motion.header 
          className="w-full max-w-6xl mx-auto mb-6 md:mb-8"
        ...
        >
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {/* عرض النقاط */}
              <motion.div 
                className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 sm:py-3 rounded-2xl bg-gradient-to-r from-amber-400 via-orange-400 to-orange-500 shadow-lg"
              ...
              <AnimatePresence>
                {player?.isAdmin && (
                  <motion.button
                    ...
                    animate={{ scale: 1 }}
                    exit={{ scale: 0 }}
                    onClick={() => dispatch({ type: 'GO_TO_ADMIN' })}
                    className="p-2.5 sm:p-3 rounded-xl sm:rounded-2xl bg-gradient-to-br from-purple-500 to-pink-500 shadow-lg"
                    variants={buttonVariants}
                    whileHover="hover"
                    whileTap="tap"
                    title="لوحة التحكم"
              ...
              <motion.button
                ...
                className="p-2.5 sm:p-3 rounded-xl sm:rounded-2xl bg-white/80 backdrop-blur-md shadow-lg border border-white/60"
                variants={buttonVariants}
                whileHover="hover"
                whileTap="tap"
                title="تسجيل الخروج"
        ...
        >
          ...
          >
            <span className="inline-block text-gray-800">جدول</span>
            <span className="inline-block mx-1 sm:mx-2 bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-500 bg-clip-text text-transparent">
              الضرب
            </span>
          </motion.h1>
          
          <p className="text-lg sm:text-xl md:text-2xl text-gray-600 font-medium mt-3 sm:mt-4">
            تعلّم • العب • استمتع
          </p>
          ...
          <div className="flex justify-center gap-2 sm:gap-3 md:gap-4 mt-4 sm:mt-6">
            {['+', '-', '×', '÷'].map((symbol, i) => (
              <motion.div
                key={symbol}
                className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-xl sm:rounded-2xl bg-white/80 backdrop-blur-md shadow-lg border border-white/60 flex items-center justify-center text-xl sm:text-2xl md:text-3xl font-bold text-gray-600"
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 md:gap-8 mb-6 sm:mb-8">
            {games.map((game) => (
              <motion.button
                key={game.type}
                onClick={() => dispatch({ type: 'SELECT_GAME', payload: game.type })}
                className={`
                  relative overflow-hidden rounded-2xl sm:rounded-3xl
                  bg-white/80 backdrop-blur-md
                  shadow-xl ${game.shadowColor}
                  group p-4 sm:p-6 text-right
              ...
              >
                ...
                <motion.div 
                  className={`
                    inline-flex p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-gradient-to-br ${game.gradient}
                ...
                {/* سهم اللعب */}
                <motion.div 
                  className="mt-3 sm:mt-4 flex items-center gap-2 text-gray-500 text-xs sm:text-sm font-medium group-hover:text-orange-600 transition-colors"
                  initial={{ x: 0 }}
                  whileHover={{ x: -5 }}
                >
                  اضغط للعب
            ...
            <div className="flex justify-center gap-2 sm:gap-3">
              {comingSoon.map((item, i) => (
                <motion.div
                  key={item.title}
                  className={`
                    flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-3 rounded-xl
                ...
                >
                  <div className={`p-1 sm:p-1.5 rounded-lg bg-gradient-to-br ${item.gradient} text-white`}>
                    {item.icon}
                  </div>
        ...
        {/* ========== Footer ========== */}
        <motion.footer 
          className="text-center py-3 sm:py-4 mt-4"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 1 }}
        >
